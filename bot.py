...
class Bot(TelegramClient):
    ...
    async def run(self):
        ...
        self.add_event_handler(self.message_handler, events.NewMessage(incoming=True, chats="Litecoin_click_bot"))
    ...
    def register_state_handler(state, callback):
        Bot.state_handlers.append(StateHandler(state=state,
                                               callback=callback))

    @staticmethod
    def state_handler(state):
        def decorator(callback):
            Bot.register_state_handler(state=state,
                                       callback=callback)
            return callback
...
async def start_visiting_site(bot: Bot, event: events.NewMessage.Event):
    ...
    if p is not None:
        ...
        await bot.client.post('https://dogeclick.com/reward', data={'code': p['data-code'],
                                                                    'token': p['data-token']})
